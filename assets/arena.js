...
const setBasics = (data) => {
  ...
  channelDescription.forEach(
    (element) =>
      (element.innerHTML = window
        .markdownit()
        .render(data.metadata.description))
  );
  channelCount.forEach(
    (element) => (element.innerHTML = `${data.length} blocks`)
  );
  ...
  const renderUser = (user, id) => {
    ...
    containers.forEach((container) => {
      ...
      let elements = ["avatar", "fullName", "link"];

      // This could be a function, with the one below.
      elements = Object.assign(
        {},
        ...elements.map((string) => ({
          [string]: template.querySelectorAll(
      ...
      elements.avatar.forEach((element) =>
        user.avatar_image.display
          ? (element.src = user.avatar_image.display
              .replace("/medium_", "/large_")
              .replace("&s=150", "&s=400"))
          : element.remove()
      );
      elements.fullName.forEach((element) =>
        user.full_name ? (element.innerHTML = user.full_name) : element.remove()
      );
      elements.link.forEach((element) =>
        user.slug
          ? (element.href = `https://www.are.na/${user.slug}`)
          : element.remove()
      );
  ...
  data.collaborators.forEach((user) =>
    renderUser(user, "channel-collaborator")
  );
};
...
const renderBlock = (block, type) => {
  ...
  type.containers.forEach((container) => {
    ...
    const srcOrSrcset = (element, size) =>
      element.tagName == "IMG"
        ? (element.src = block.image[size].url)
        : (element.srcset = block.image[size].url);

    elements.title.forEach((element) =>
      block.title ? (element.innerHTML = block.title) : element.remove()
    );
    elements.imageThumb.forEach((element) =>
      block.image ? srcOrSrcset(element, "thumb") : element.remove()
    );
    elements.imageSquare.forEach((element) =>
      block.image ? srcOrSrcset(element, "square") : element.remove()
    );
    elements.imageDisplay.forEach((element) =>
      block.image ? srcOrSrcset(element, "display") : element.remove()
    );
    elements.image.forEach((element) => {
      block.image ? srcOrSrcset(element, "large") : element.remove();
    });
    elements.embed.forEach((element) => {
      block.embed ? (element.innerHTML = block.embed.html) : element.remove();
    });
    elements.audio.forEach((element) => {
      block.attachment
        ? (element.src = block.attachment.url)
        : element.remove();
    });
    elements.video.forEach((element) =>
      block.attachment ? (element.src = block.attachment.url) : element.remove()
    );
    elements.link.forEach((element) => {
      if (block.source) {
        element.href = block.source.url;
        elements.linkTitle.forEach(
          (element) => (element.innerHTML = block.source.title)
        );
      } else if (block.attachment) {
        element.href = block.attachment.url;
        elements.linkTitle.forEach(
          (element) => (element.innerHTML = block.title)
        );
      } else {
        element.remove();
      }
    ...
    elements.description.forEach((element) =>
      block.description_html
        ? (element.innerHTML = block.description_html)
        : element.remove()
    );
    elements.type.forEach((element) => (element.innerHTML = type.name));
    elements.timeUpdated.forEach(
      (element) =>
        (element.innerHTML = `Updated ${showRelativeDate(block.updated_at)}`)
    );
    elements.timeCreated.forEach(
      (element) =>
        (element.innerHTML = `Created ${showRelativeDate(block.created_at)}`)
    );
